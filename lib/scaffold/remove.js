...
/**
 * Will remove a service from wificoin-node.json
...
/**
 * Will remove the Node.js service and from the wificoin-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The wificoin-node configuration directory
...
function remove(options, done) {
  ...
  var bitcoreConfigPath = path.resolve(configPath, 'wificoin-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(bitcoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a wificoin-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to wificoin-node.json

...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for wificoind is necessary for this upgrade with the "reindex=1" wificoin.conf option. \n' +
                 'There are changes necessary in both wificoin.conf and wificoin-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/bitpay/wificoin-node/blob/wificoind/docs/upgrade.md \n');
 ...
 * @param {Object} options.config - The parsed wificoin-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './wificoin-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in wificoin-node services directory
 ...
 * @param {Node} node - The Wificoin Node instance
 */

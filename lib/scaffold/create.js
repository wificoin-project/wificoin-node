...
var BASE_PACKAGE = {
  description: 'A full Wificoin node build with Bitcore',
  repository: 'https://github.com/user/project',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
    'wificoin-lib': '^' + bitcore.version,
    'wificoin-node': version
  }
};

/**
 * Will create a directory and wificoin.conf file for Wificoin.
 * @param {String} dataDir - The absolute path
 * @param {Function} done - The callback function called when finished
 */
function createWificoinDirectory(datadir, done) {
...
/**
 * Will create a base Wificoin Node configuration directory and files.
 * @param {Object} options
 * @param {String} options.network - "testnet" or "livenet"
 * @param {String} options.datadir - The wificoin database directory
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/wificoin-node.json', configJSON);
      if (!isGlobal) {
...
/**
 * Will setup a directory with a Wificoin Node directory, configuration file,
 * wificoin configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the bitcore node configuration directory
 * @param {String} options.datadir - The path to the wificoin datadir
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the wificoin-node directory and configuration
    ...
    function(next) {
      // Setup the wificoin directory and configuration
      if (!fs.existsSync(absDataDir)) {
        createWificoinDirectory(absDataDir, next);
      } else {
        next();
      }
    },

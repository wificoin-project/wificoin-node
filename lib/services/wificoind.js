...
/**
 * Provides a friendly event driven API to wificoind in Node.js. Manages starting and
 * stopping wificoind as a child process for application support, as well as connecting
 * to multiple wificoind processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Wificoin(options) {
  if (!(this instanceof Wificoin)) {
    return new Wificoin(options);
  }
...
util.inherits(Wificoin, Service);

Wificoin.dependencies = [];

Wificoin.DEFAULT_MAX_TXIDS = 1000;
Wificoin.DEFAULT_MAX_HISTORY = 50;
Wificoin.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Wificoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Wificoin.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Wificoin.DEFAULT_SPAWN_RESTART_TIME = 5000;
Wificoin.DEFAULT_SPAWN_STOP_TIME = 10000;
Wificoin.DEFAULT_TRY_ALL_INTERVAL = 1000;
Wificoin.DEFAULT_REINDEX_INTERVAL = 10000;
Wificoin.DEFAULT_START_RETRY_INTERVAL = 5000;
Wificoin.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Wificoin.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Wificoin.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
  ...
  zmqpubhashblock: 'tcp://127.0.0.1:28665',
...
Wificoin.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Wificoin.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Wificoin.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || Wificoin.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Wificoin.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Wificoin.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Wificoin.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Wificoin.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Wificoin.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Wificoin.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Wificoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Wificoin.prototype._initCaches = function() {
...
Wificoin.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Wificoin.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
Wificoin.prototype.getPublishEvents = function() {
  return [
    {
...
Wificoin.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'wificoind/' + name, 'total:', this.subscriptions[name].length);
};

Wificoin.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'wificoind/' + name, 'total:', this.subscriptions[name].length);
};

Wificoin.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'wificoind/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Wificoin.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'wificoind/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Wificoin.prototype.unsubscribeAddressAll = function(emitter) {
...
Wificoin.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Wificoin.DEFAULT_CONFIG_SETTINGS;
...
Wificoin.prototype._parseWificoinConf = function(configPath) {
  var options = {};
...
Wificoin.prototype._expandRelativeDatadir = function() {
...
Wificoin.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in wificoind config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in wificoind config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in wificoind config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './wificoin.conf');

  log.info('Using wificoin config file:', configPath);
  ...
  _.extend(this.spawn.config, this._parseWificoinConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseWificoinConf(networkConfigPath));
  }
...
Wificoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of wificoin-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of wificoin-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of wificoin-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to wificoind from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from wificoind. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from wificoind. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in wificoin.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that wificoind is undergoing a reindex. ' +
             ...
             'of wificoin-node services will start.');
    node._reindex = true;
  }
};

Wificoin.prototype._resetCaches = function() {
...
Wificoin.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Wificoin.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Wificoin.prototype._initChain = function(callback) {
  var self = this;
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          self.emit('ready');
          log.info('Wificoin Daemon Ready');
          callback();
        });
      });
...
Wificoin.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 9665
  };
  if (this.node.network === bitcore.Networks.testnet) {
    networkOptions.rpcport = 18665;
  }
  return networkOptions;
};

Wificoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/wificoin.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/wificoin.conf';
    }
  }
  return networkPath;
};

Wificoin.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === bitcore.Networks.testnet) {
...
Wificoin.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('wificoind/hashblock', message.toString('hex'));
    }
  }

};

Wificoin.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Wificoin.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Wificoin Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Wificoin.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Wificoin.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('wificoind/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Wificoin.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('wificoind/rawtransaction', message.toString('hex'));
    }
...
Wificoin.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Wificoin.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Wificoin.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Wificoin.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
Wificoin.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('Wificoin Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || Wificoin.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

Wificoin.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Wificoin.prototype._stopSpawnedWificoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/wificoind.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned wificoin process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean wificoin process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Wificoin.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedWificoin(function(err) {
    if (err) {
      return callback(err);
    }

    log.info('Starting wificoin process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Wificoin process unexpectedly exited with code:', code);
        log.warn('Restarting wificoin child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Wificoin process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn wificoind.'));
      }
...
Wificoin.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to wificoind.'));
    }
 ...
 */
Wificoin.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Wificoin configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Wificoin.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Wificoin.prototype.syncPercentage = function(callback) {
  var self = this;
...
Wificoin.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {Function} callback
 */
Wificoin.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Wificoin.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
Wificoin.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Wificoin.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Wificoin.prototype._getHeightRangeQuery = function(options, clone) {
 ...
 * @param {Function} callback
 */
Wificoin.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
Wificoin.prototype._getConfirmationsDetail = function(transaction) {
...
Wificoin.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Wificoin.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Wificoin.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A wificoin transaction id
 * @param {Function} callback
 */
Wificoin.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Wificoin.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
Wificoin.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
Wificoin.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Wificoin.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
Wificoin.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Wificoin.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Wificoin.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Wificoin.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
Wificoin.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Wificoin.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Wificoin.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Wificoin.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Wificoin.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Wificoin.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Wificoin.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Wificoin.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Wificoin.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Wificoin.prototype.getInfo = function(callback) {
  var self = this;
...
Wificoin.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Wificoin.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('wificoind spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('wificoind process did not exit'));
      }
